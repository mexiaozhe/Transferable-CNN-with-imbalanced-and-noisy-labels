...
class pspnet(nn.Module):
    ...
    def __init__(self, n_classes=11):
        ...
        self.conv1 = nn.Conv2d(9, 64, kernel_size=7, stride=2, padding=3, dilation=1, bias=False) #, trackrunningstats=True
        ...
        self.classification = nn.Conv2d(512, n_classes, 1, 1, 0, 1)
    ...
    def forward(self, x):
        ...
        ndwi = (x0[:,1,:,:] - x0[:,3,:,:]) / (x0[:,1,:,:] + x0[:,3,:,:])
        ndwi = t.tensor(ndwi.reshape(1, ndwi.shape[0], ndwi.shape[1], ndwi.shape[2])).cuda().transpose(0,1)
        ndbi = (x0[:,4,:,:] - x0[:,3,:,:]) / (x0[:,4,:,:] + x0[:,3,:,:])
        ndbi = t.tensor(ndbi.reshape(1, ndbi.shape[0], ndbi.shape[1], ndbi.shape[2])).cuda().transpose(0,1)
        ndvi = t.cat([ndvi,ndwi,ndbi],1)
